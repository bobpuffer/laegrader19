...
//            $rangesdisplaytype   = $this->get_pref('rangesdisplaytype');
        	$rangesdisplaytype   = GRADE_DISPLAY_TYPE_REAL;
...
//                   }
					// calculate up the non-points portion first before changing values to correctly calculate points
                    if ($item->needsupdate) {
...
//						                    grade_format_gradevalue_real($value, $grade_item, $decimals, $localized) . ')';
						        $nonpointvalue = grade_format_gradevalue_percentage($gradeval, $item, $decimalpoints, $localized);
								break;
...
//						                    grade_format_gradevalue_real($value, $grade_item, $decimals, $localized) . ')';
						        $nonpointvalue = grade_format_gradevalue_letter($gradeval, $item);
								break;						                    
						    case GRADE_DISPLAY_TYPE_LETTER_PERCENTAGE:
						        $nonpointvalue =  grade_format_gradevalue_letter($gradeval, $item) . ' (' .
						                    grade_format_gradevalue_percentage($gradeval, $item, $decimalpoints, $localized) . ')';
								break;
						    case GRADE_DISPLAY_TYPE_PERCENTAGE_LETTER:
						        $nonpointvalue =  grade_format_gradevalue_percentage($gradeval, $item, $decimalpoints, $localized) . ' (' .
						                    grade_format_gradevalue_letter($gradeval, $item) . ')';
						        break;
						}
						// if we have an accumulated total points that's not accurately reflected in the db, then we want to display the ACCURATE number
	                    // we only need to take the extra calculation into account if points display since percent and letter are accurate by their nature
	                    // If the settings don't call for ACCURATE point totals ($this->accuratetotals) then there will be no earned_total value
						$display_array = array(1,12,13,21,31); // if displaytype includes points
	                    $tempmax = $item->grademax;
						if ( in_array($gradedisplaytype, $display_array) && isset($this->grades[$userid][$grade->itemid]->cat_item)) {
	                       	$items = $this->gtree->items;
		               		$grade_values = $this->grades[$userid][$grade->itemid]->cat_item;
		               		$grade_maxes = $this->grades[$userid][$grade->itemid]->cat_max;
		               		$this_cat = $this->gtree->items[$grade->itemid]->get_item_category();
					    ...
					    $pointvalue = grade_format_gradevalue_real($gradeval, $item, $decimalpoints, $localized);
						$item->grademax = $tempmax;
	                    ...
	                    switch($gradedisplaytype) {
	                    	case 12:
	                    	case 13:
			                    $studentshtml .= '<span class="gradevalue'.$hidden.$gradepass.'">' . $pointvalue . ' (' . $nonpointvalue . ') </span>';
								break;
	                    	case 21:
	                    	case 31:
			                    $studentshtml .= '<span class="gradevalue'.$hidden.$gradepass.'">' . $nonpointvalue . ' (' . $pointvalue . ') </span>';
								break;
	                    	case 1:              		
			                    $studentshtml .= '<span class="gradevalue'.$hidden.$gradepass.'">' . $pointvalue . '</span>';
								break;
	                    	default:
			                    $studentshtml .= '<span class="gradevalue'.$hidden.$gradepass.'">' . $nonpointvalue . '</span>';
								break;
	                    }
					}
                }
